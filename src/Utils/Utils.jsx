...
export function validarSoloLetrasYTildes(val) {
  // Permitir que el campo esté vacío
  if (val === "") return true;

  // Expresión regular para letras con y sin tildes (mayúsculas y minúsculas)
  const regex = /^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/;

  // Verificar si el valor cumple con la expresión regular
  return regex.test(val);
}
export function esSoloNumeros(cadena) {
  const regex = /^\d*$/;
...
export const getPlanColor = (plan) => {
  if (plan === 1) {
    return "#003540e6"; // avanzado
  } else if (plan % 2 === 0) {
    return "#41D171"; // premium
  } else if (plan % 2 !== 0) {
    return "#94A4C4"; // basico
  } else {
...
export const transformToDate = (dateString) => {
  if (!dateString) return null;
